...
@testset ExtendedTestSet "treewidth" begin
    @testset  "subset function" begin
        A = [1,2,3]
        B = [1,2,3,4]
    ...
    @testset  "local_circuit_graph" begin
        N = 4
        G = Graph(N)
        add_edge!(G, 1, 2)
        add_edge!(G, 2, 3)
    ...
    @testset  "interaction_graph" begin
        ## Interaction graph:
    ...
    @testset  "lacking_for_clique_neigh" begin
        G = complete_graph(5)
    ...
    @testset  "is_tree_decomposition" begin
    ...
    @testset  "contraction_order" begin
        TN0 = test_setup()
        ## Contraction order:
        TN = copy(TN0)
    ...
    @testset  "optimize_contraction_order!" begin
