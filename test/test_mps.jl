...
@testset ExtendedTestSet "mps" begin
    @testset  "contract_svd_mps" begin
        T = Tensor(rand(2,2,2))
        mps = OpenMPS(T, 3)
    ...
    @testset  "check_mps exceptions" begin
        T = Tensor(rand(2,2,2))
        mps = OpenMPS(T, 3)
    ...
    @testset  "conversion from state-vector to mps" begin
        N = 5
    ...
    @testset  "conversion from state-vector to mps exceptions" begin
        ψ = randn(ComplexF64, 6)

        @test_throws ErrorException("Input state must have length 2^N") MPS(ψ)
    end

    @testset  "mps constructor" begin
        N = 3
    ...
    @testset  "open_mps exceptions" begin
    ...
    @testset  "closed_mps exceptions" begin
    ...
    @testset  "periodic_mps exceptions" begin
        T = Tensor(rand(2,2,2))
        mps = PeriodicMPS(T, 3)
        @test_throws ErrorException contract_svd_mps(mps, er=0.0)
    end

    @testset  "test permute MPS" begin
        N = 6
    ...
    @testset  "test permute MPS exceptions" begin
    ...
    @testset  "test switch MPS qubits" begin
        N = 6
        b = AbstractArray{ComplexF64}[]
        for i in 1:N
            push!(b, randn(ComplexF64, (2)))
    ...
    @testset  "test switch MPS exceptions" begin
    ...
    @testset  "test switch neighboring MPS qubits" begin
        N = 6
        b = AbstractArray{ComplexF64}[]
        for i in 1:N
            push!(b, randn(ComplexF64, (2)))
    ...
    @testset  "test switch neighboring MPS qubits exceptions" begin
        N = 6
        b = AbstractArray{ComplexF64}[]
        for i in 1:N
            push!(b, randn(ComplexF64, (2)))
