...
- er: maximum error in the truncation
"""
function truncate_svd(T1::Tensor, T2::Tensor, er)
    ...
    if size1[end] <= prod(size1[1:end-1]) || size2[1] <= prod(size2[2:end])
        return T1, T2
    end
    ...
    truncation_error = 0
    k1 = 0
    for (i,e) in enumerate(reverse(S))
        truncation_error += e
        if truncation_error > er
            k1 = length(S) - i + 1
            break
        end
    end

    max_bond_dim = minimum([k1, length(S)])
