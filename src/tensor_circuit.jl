...
effectively applying the gate.
"""
function tensor_circuit!(ψ::MPS, cg::CircuitGate{M,N,G}; max_bond_dim=typemax(Int64), revert=true, openidx=nothing) where {M,N,G}
    # TODO: specialization for various G
    ...
    for (i, w) in enumerate(cg.iwire[2:end-1])
        ...
        for (i,w) in enumerate(ψ.tensors)
            ...
            if shape[1] > max_bond_dim
                ψ.tensors[i-1], ψ.tensors[i] = truncate_svd(ψ.tensors[i-1], ψ.tensors[i], max_bond_dim)
            elseif shape[end] > max_bond_dim
                ψ.tensors[i], ψ.tensors[i+1] = truncate_svd(ψ.tensors[i], ψ.tensors[i+1], max_bond_dim)
            end
        end
